...
class Registry {
    // protected common:
    constructor(name, _canMangleName = null) {
        this.name = name;
        this._canMangleName = _canMangleName;
    ...
    register(ruleset) {
        ...
        if (ruleset.groupBy.kind === "font") {
            this.fontFaces[ruleset.groupBy.id] = ruleset.entries;
        }
        else {
    ...
    isSimlePrimarySelector(selector) {
        let spaceIndex = -1;
        let part;
